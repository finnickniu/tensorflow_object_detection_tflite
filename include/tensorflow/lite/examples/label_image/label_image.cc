...
cv::Mat threshold_img(cv::Mat rgb_img)
{
 
  cv::Mat out_put_img;

  cv::Size sz = rgb_img.size();
  auto h = sz.height;
  auto w = sz.width;
	cv::cvtColor(rgb_img, rgb_img, cv::COLOR_BGR2HSV);
  cv::medianBlur(rgb_img,rgb_img,5);
  cv::Scalar lower_green=cv::Scalar(55,10,10);
  cv::Scalar upper_green=cv::Scalar(90,255,255);
  cv::inRange(rgb_img,lower_green,upper_green,rgb_img);
  auto erosion_type =cv::MORPH_RECT;
  cv::Mat element = cv::getStructuringElement(erosion_type,cv::Size(9,9));
  cv::dilate(rgb_img,rgb_img,element);
  return rgb_img;
}
...
void RunInference(Settings* s) {
  ...
  // auto image = cv::imread("/home/apptech/tensorflow/c++/testdata/2.bmp", 0 );
  ...
  auto output1 = interpreter->outputs();

  for (int i; i< output1.size(); ++i){
    std::cout << "class" <<output1[i] <<std::endl;
  }
  ...
  auto output_data1 = output_scores->data.f;

  for (int i = 0; i < 100; i++){
    auto output = output_data1[i];
    std::cout << "class: "<< output << std::endl;

    // if (i % 2 == 0){
    //   std::cout << "location: "<< output*834 << std::endl;
    // }
    // else{
    //   std::cout << "location: "<< output*416 << std::endl;
  ...
  // const float* detection_locations = TensorData<float>(output_locations, 0);
  // const float* detection_classes = TensorData<float>(output_classes, 0);
  // const float* detection_scores = TensorData<float>(output_scores, 0);


  // for (int i = 0; i < output_locations.size(); i++){
  //   auto output = output_locations[i];
  ...
  //   std::cout << "class: "<< result.second << std::endl;
...
int Main(int argc, char** argv) {
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case 'r':
        s.number_of_results =
            strtol(optarg, nullptr, 20);  // NOLINT(runtime/deprecated_fn)
        break;
      case 's':
